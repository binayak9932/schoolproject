...
def displayAllCustomer():
 if myConnection:
  ...
  if data:
   ...
   pk=pd.DataFrame(data,columns=["Customer ID","Customer Name","Address","Phone"])
   print(pk)
  else:
 ...
 if myConnection:
  ...
  if data:
   createTable ="""CREATE TABLE IF NOT EXISTS ACCOUNT(CID VARCHAR(10),ACCOUNT_NO INT
    PRIMARY KEY,ACCOUNT_TYPE VARCHAR(20) NOT NULL ,AMOUNT INT NOT NULL , PIN INT NOT NULL
     UNIQUE)"""
   cursor.execute(createTable)
...
def displayAllAccounts():
 ...
 if myConnection:
  ...
  if data:
   ...
   pk=pd.DataFrame(data,columns=["Customer ID","Account number","Type","Amount","Pin"])
   print(pk)
  else:
